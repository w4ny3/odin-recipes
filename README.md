October 2025
This is one of my first attempts at building a simple website with simple html. The goal was to create a webpage with 3 recipes and practice using ordered and unordered lists, links, and photos. Also, there was the added benefit of using git commands to push the files to Github.

November 2025
After learning how to use external CSS, I created an external CSS for the recipe website to try out a few things I have learned.

November 2025 Update 2
Collaborator Jason Barrow created a pull request to add Nunna's Meatballs to the project. Changes were pushed to main branch.